...
# чисел, отсортированных по убыванию? Решил, что так было бы слишком просто -- ведь можно было бы размещать новый
# элемент  просто в конец рейтинга, а потом сортировать рейтинг при помощи print(sorted(ratings, reverse=True)), так
# что сделал рандомный порядок значений.
#
# 2) Непонятно, почему в подсказке при вводе пользователем числа 8 оно размещается в начале рейтинга, а при вводе числа
# 1 оно размещается в конце рейтинга. (Возможно, порядок значений всё-таки должен быть убывающим...) Решил размещать
# всё в начале рейтинга.
